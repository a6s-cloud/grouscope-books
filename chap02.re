...
プロジェクトに参加したはいいもののコミットできないというメンバの中には、環境構築ができない、または億劫だという人が多くいます。
...
これまでの節では、ミーティングを行い、環境構築を素早く終了することで、明確な方針を立てて素早く開発に取り掛かるTIPSを解説しました。
...
私はこの問題点を整理すると、次の二つの原因に整理できると考えています。
一つは、調査がなかなか進まずその方面の開発が滞ってしまう場合があること。また、それを取り入れるかどうかの議論が適切な頻度、タイミングで行われず、方針が定まるのに時間がかかってしまう場合があること。(開発準備の遅延)
二つ目は、議論に参加していなかったメンバが新しい方針についていけず、その方面の開発にコミットできるメンバが少なくなってしまう可能性があることです。(認識のズレの発生)

これらは、ミーティングを再度行うことによって解決できる問題です。
しかし、ミーティングを毎週のように行うということは開発メンバにとって非常にコストになりますし、面倒です。
そこでこのような問題に対応するTIPSとして提案したいのは、生じた課題に対して議論するスペースを作ることです。
私たちのチームの場合、GitHubに課題調査用または議論用のissueを起票し、そこで議論の記録を残すようなルールを定めています。
下図は、PHPからPythonのバッチを叩く機能を実装する際にその役割分担や、開発の方針決定のための調査を行った際に起票されたissueです。

//image[chap02/003_discussIssue][ミーティング中に決まりきらなかった役割分担を議論するissue]

このようにissueを起票し、その課題に対する調査や結果をissueに記載していくことによって多くのメリットが生まれます。
...
しっかり議論の経緯と結果を残すことによって、実際に開発に取り掛かる際に参考資料として参照するためのよいリソースを作ることができ、開発ハードルを下げることができるのです。
...
そういった場合は、恐れず正直に今ぶつかっている困難を開発メンバに質問するのがよいTIPSになります。

例えば、普段は利用していないフレームワークを利用しているときに記述の仕方がわからなくなってしまった場合、どのようなことで悩んでいるかをSlackなどで相談するのがいいでしょう。
...
ちなみにこれは、Linuxで使えるコマンドがMacOSの場合は使えないためにエラーが発生しているというものでしたが、私自身がBashを読むのが得意でないこと、OS操作に詳しくないことなどからなかなか解決することができませんでした。
...
'issue駆動プロジェクトマネジメント'はどのタイミングからでも非常に簡単にプロジェクトに取り入れることができます。
