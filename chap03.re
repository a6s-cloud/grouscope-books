...
=={sec-ext0} はじめに
現在、開発効率を高めてくれる便利ツールやサービスが沢山あります。
しかし、沢山あるからこそ何を使えば良いのか迷ってしまう事もあるでしょう。
...
図を作成するにあたっての大きな問題点は、メンテナンスが難しいことです。
シーケンスやER図は追加や更新が発生します。そのため、手軽に修正できるようにしなければなりません。
plantUMLはコードベースのため、画像加工ツールではなくエディタで修正するので、メンテナンスが容易です。
...
==== Draw.io
Draw.ioとはフローチャートやシステム構成図、ネットワーク図を作成できる無料の作図サービスです。
...
チームで利用すると有料になりますが、個人利用なら無料で使用できます。

ワイヤーフレームを作成する際に使用しましたが、個人的に使い勝手はDraw.ioのほうが好みです。

==== Keynote、Googleスライド
既に皆さんもよくご存知かと思いますが、プレゼンテーションツールです。
チーム開発の目的やペルソナ、ちょっとしたUIを作成する際に使用しました。
GoogleスライドだとURLリンクを発行して、他のメンバーにも共有できます。

==== Googleスプレッドシート
こちらもよく知られている表計算ツールです。
DBのテーブル構成を考える時に使用しました。
しかし、GitHubで管理できないので、修正漏れが発生する可能性はあります。

//image[sec-ext1-2][テーブル構成][scale=1.0]

==== Swagger UI
API仕様書を作成できるツールです。
SPA構成の場合は先にAPI仕様を決めることで、フロントエンド、バックエンドを並行して作業を進めることができます。

JSONもしくはYAML形式で記載できますが、JSONはコメントが書けません。
ネストが深くなると可読性が下がるのでYAML形式で書くことをおすすめします。

//image[sec-ext1-3][API仕様書][scale=0.8]

=={sec-ext2} ナレッジ共有
チーム開発では作業を属人化させないこと、ナレッジを皆に共有することが大事です。
そのため、環境構築手順やツールの使い方などをドキュメントに残すようにしましょう。
...
ただ、画像を入れる際はpathを指定しなければならないのが少し手間ではあります。

==== GitHub Wiki
GitHubにはWikiのページがありますが、編集がしづらい、差分が分かりづらい、画像を入れづらいと感じる場面があり、個人的には使いづらかったです。
 ...
 * grouscope_netlify
 ** netlifyのデプロイ状況の通知用です。issueやコメント、プルリクの作成時に通知するようにしています。
 * grouscope_デザイン
 ** 画面デザインのについて話し合う際に使用するチャンネルです。
 * grouscope_ミーティング
 ** 対面やオンラインでMTGする際にスケジュール調整をしたり、議事録を残したりするチャンネルです。
 * grouscope_開発
 ** プロジェクトの開発に関する話をするチャンネルです。
 * grouscope_kibela
...
Organizationsアカウントで作成したリポジトリのみ対象になっています。
Qiitaに詳細を書いておりますので興味がある方は確認してみてください。
...
こちらもQiitaに詳細を書いておりますので興味がある方は確認してみてください。
...
僕たちは最初、個人アカウントでリポジトリを作成し、メンバーにcollaboration権限を付与するという方法にしていました。
しかし、リポジトリを作成するたびにcollaboration権限を付与する作業は手間に感じることが多々ありました。
そのため、途中からorganizationアカウントに移管しました。
organizationにするとプロフィールにorganizationのアイコンが表示され、モチベーションも上がるのでおすすめです。

==== Projects
GitHubのProjectsとはカンバン形式のタスクボードです。
これを使用することで現在の進捗がひと目で分かるようになります。
issueやpullrequestにも対応しているのでcloseしたらissueを閉じるなど自動で運用することが可能です。
