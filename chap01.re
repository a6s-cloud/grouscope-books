...
上記のようなストレス、かつ信頼できるメンバーと一緒に開発できれば楽しいだろうなと考えていました。
...
フリーランスになってから、特定の会社や現場でのみ通用するスキル・経験はリスクと強く考えるようになりました。
しかし、今の自分のスキル・経験が他の現場で通用するのかを確認する術はなかなかありません。
...
そしてモチベーションが下がっていくというサイクルに陥っていました。
良くも悪くも、個人開発は途中で諦めても他の人に迷惑が掛かりません。
そこに甘えて、投げ出してしまったプロジェクトが沢山あります。
個人開発ではやり切ることができなかったので、チーム開発で他の人も巻き込み、良い意味で諦めづらい空気を作りたいと考えていました。
...
これは1番手軽に声を掛けられそうですが、自分と全く関わりのない人から返信がくる可能性もあります。
その人の性格や方法が分からないので、見ず知らずの人といきなり始めるのは不安があります。
...
つとむさんはGitHubやQiitaに積極的にアウトプットをし、一緒に開発をすることができたら楽しいだろうと思っていました。
...
何事もモチベーションとスピードが大事なので、すぐに対面で進め方を決めていくことにしました。
...
少しだけ、僕たちが開発中のアプリの説明をすると勉強会のツイートを集計・分析して、可視化することで勉強会の雰囲気や特徴を掴めるようにするものです。
勉強会の特徴を把握することで、初めて参加する時の不安などを減らし、勉強会への参加ハードルを下げることを目的としたものです。現状だとプロトタイプまでは作成できた状態ですが、UIは一新して作り直そうと考えています。

==== 設計を決める
最初に決めたことはUI部分です。画面から考えていくのが１番イメージしやすいと思ったからです。
しかし、デザインスキルが低い僕にはUI設計に苦戦しました。また、アプリの機能イメージも皆に伝えきれませんでした。
...
コードを書いて手を動かせばモチベーションも上がるはずだとこの時は甘く考えていました。

=={sec1-ext5} モチベーションを維持することは難しい
実際にコードを書き始めるまでは、手を動かしてコードを書き始めることでモチベーションも上がっていくだろうと思っていました。
最初は勢いよく進んでいきましたが、2週間も経つと、自分含め皆のモチベーションも下がってきているのを感じました。
個人開発ならば、僕はここで諦めていたでしょう。

しかし、今回はチーム開発です。
発起者として責任もあるため、僕が簡単には投げだしてしまうわけにはいきません。
...
そうして後回しにしているうちに、当然チーム開発に触れる時間が少なくなり、進捗も悪くなるのでモチベーションが下がってしまいます。

==== タスクを明確しなかったため、何をしていいか分からない
...
特に平日は業務の披露もあるため、上手く頭が働かず余計に時間が掛かってしまいます。
僕は作業の段取りを最初にしっかり組まずに作業を初めてしまったため、作業毎に何をすれば良いのか都度考えるという非効率な進め方になってしまいました。
...
例えば、設計作業を2週間で終わらせるとしましょう。期限までに作業を完了させるためには1週間でここまで完了していないといけない、3日目の進捗でこのタスクを終わらせなければならないというように、逆算して計画をし、作業を着実に進めていく必要があるため責任をもって作業に取り組むことができます。
...
タスクを明確に決めていなかったので、実装途中に機能をどんどん追加し、収集が付かなくなってしまい進捗をアウトプットできなくなってしまいました。
進捗をアウトプットできないので前に進んでいる感が生まれないのです。

大きな目標だけを立てて、全力疾走するのではなく中間目標を立てて、段階的に目標をクリアしていくことで高いモチベーションを保ちやすいと気づき、途中からはタスクの進め方を変えています。
...
チーム開発だと自分の作業がクリティカルパスとなり、全体の作業が止まってしまうことがあるため、作業の分担を決める際に得意な人にやってもらうという空気が生まれてきます。僕たちも得意分野で作業分担を決めてしまい、進みが早いと周りのメンバーがキャッチアップしづらい状況になってしまい、属人化のような形になりました。
...
例えばリーダが皆を引っ張っていくスタイルや皆で定期的に集まり進捗確認していくスタイルです。
しかし、人によってリーダの向き不向きがあると思います。僕たちの場合は対面でなく、リモートの作業が主であるためリーダが引っ張っていく形では上手くいかないのではと考えました。
...
アイコンや理念、構成図、アウトプット記事などのアピールできるものを集約して載せるようにしました。アウトプットを一箇所に集めることで、成果が目に見えて分かるようになり達成感を感じることができます。そして、もっとREADMEを豪華にしたい欲求を掻き立てる作戦です。
...
今回、bashによる環境の自動作成やDockerによる管理などの環境構築周りをつとむさんに作成してもらいました。この辺りの内容を僕はまったく理解できていませんでしたが、実践ベースで勉強することができてスキルアップすることが出来ました。
...
==== モチベーション
個人開発もチーム開発もモチベーションについてはあまりはありませんでした。
最初は熱が高く、時間が経つにつれて下がってきます。

チーム開発は途中で投げ出すと周りの人に迷惑を掛けてしまうので、粘り強くなりました。

しかし、周りの人が進めている中で自分だけ作業できない状態が続くと申し訳なさからのストレスになる可能性もあります。
...
他メンバーとの話し合いがなくタスクの分割を考える必要がないためです。
...
他のメンバーのコードを読むことができ、そして自分のコードをレビューしてもらうことで、今まで自分で気づけなかったコーディングの癖や問題を気づけたり、自分では考えつかなかった良いアイディアを学ぶことができます。また、コードレビューの過程が入ることで、読みやすいコードを書くことを心掛けるようになります。
１人で実装するときより、簡潔さや分かりやすさを意識してコードを書くようになるため技術向上できる気がしています。
...
Tokiさん、つとむさんのような優秀なエンジニアが集まってチームで足並みを揃えて進めていくとなると中々上手くはいきません。
...
是非、想像だけでは終わらせずに、実際にプライベートでのチーム開発を挑戦してみてください。
