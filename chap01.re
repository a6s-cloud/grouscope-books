...
僕は、自分のコードはアンチパターンになっているのではないか？もっと良い書き方があるのではないか？と考えてしまうタイプです。
...
そもそも僕は内向的なので簡単に声を掛けるなんて恐れ多くてできなかったです。
...
いくつかアプローチ方法を考えた結果、最初は勉強会のLTで呼びかけました。
...
もっと上手くやるにはどうすればいいのかを考える必要があったので、モチベーションが下がってしまう要因を考えてみました。

==== 金銭的なインセンティブがないから？
金銭的なインセンティブがないからモチベーションが下がるのではないか？
しかし、心理学的に「金銭的報酬は、人の内発的動機づけを低下させる」という結果@<fn>{結果}がでています。
//footnote[結果][参考文献:モチベーション３．０　持続する「やる気！」をいかに引き出すか]
...
期限があると少なからずプレッシャーを感じます。そういった中だと楽しく進めることができなくなるのではと考えていましたが、期限がないことで気も緩んでしまいます。
...
アイコンや理念、構成図、アウトプット記事などのアピールできるものを集約して載せるようにしました。アウトプットを一箇所に集めることで「やっている感」がでて、もっとREADMEを豪華にしたい欲求を掻き立てる作戦です。
...
僕たちも作業をした際に、コントリビューションを増やせるような工夫しました。
コントリビューションはissueやプルリクの作成で生やすことができます。
...
0から徐々に1になり、2になり、10になりどんどん良くなっていく過程を経験できて楽しかったです。
...
例えば今回だと環境構築をもっと楽にしたい。タスクをもっと見える化したい。気軽にアウトプットできるようにしたい。などがでてきました。
...
=={sec-ext9} 最後に
プライベートでのチーム開発は想像以上に難しかったです。Tokiさん、つとむさんのような優秀なエンジニアが集まってもうまく開発は進んでいきません。チーム開発は人が単に集まっただけでは機能しません。人、仕組み、ルールなどを決めていく必要があります。それをTry & Errorで進めていき、チームとしてまとめていくのがプライベート開発の醍醐味だと感じています。

今回、僕の気持ちや考えを伝えました。そして、皆さんは「自分ならもっと上手くできる」、「私ならこんな工夫する」など考えがあると思います。
是非、想像だけでは終わらせずに、実際にプライベートでのチーム開発を実感してみてください。

//image[myuhu4][][scale=1.0]