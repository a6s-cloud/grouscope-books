...
\begin{description}
\item[config{-}starter.yml] \mbox{} \\
Starterでプロジェクトを作成したときに選んだオプション（の一部）が書かれたファイルです。ここに書かれているオプションは、プロジェクトをダウンロードしたあとからでも変更できます。
...
\begin{reviewimage}[H]%%margin_book
\includegraphics[width=0.75\maxwidth]{./images/chap01-starter/margin_book.png}%
\reviewimagecaption{ページ左右の余白幅を変更する理由}
...
ただしこの機能では、\LaTeX{}のスタイルファイル（\texttt{sty/starter.sty}や\texttt{sty/mytextsize.sty}）の中で行える範囲でしか変更はできません。それ以上のことがしたい場合は、\reviewsecref{「2.7.2 印刷用と電子用で設定を少し変えるにはどうするの？」}{sec:2-7-2}を参照してください。
