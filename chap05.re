...
==== アクセスする仕組み
そとからアクセスする時のあなたの家のIP アドレスはわかりました。それではあなたの家のPC を起動してDocker やLinux でNginx やApache といったWeb サーバを立ち上げて、その後にあなたのケータイを開いて4G/LTE 網(自宅のWifi 接続ではない)につないでください。これは家の外にあるPC 端末を想定しています。そして先程出力されたIP アドレスに対してWeb ブラウザからhttp リクエストを飛ばしてみましょう。
//cmd{
http://y.y.y.y
//}
...
//image[chap05/0062_HomServerPortForward][ルータのポートフォワードイメージ][scale=1.0]
