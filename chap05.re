...
AWS にログインしたらEC2 コンソールまでインスタンスの作成ボタンを押してインスタンスの作成に進みます。今回は紙面の都合上、この後に出てくる設定画面については説明を割愛しますがUbuntu 18.04 LTS を選択した後はディスク容量を16GB くらいに拡張子、その他はデフォルト設定のままで作成していきます。そしてインスタンス作成の最後にSSH の秘密鍵を作成してダウンロードするのを忘れないようにしてください。
//image[chap05/AWSEC2_CreateInstance0001][ディストリビューションの選択][scale=1.0]

EC2 ダッシュボードに戻ると作成したインスタンスが表示されます。このインスタンスの状態が"running" になれば準備完了です。
//image[chap05/AWSEC2_CreateInstance0002][インスタンス準備完了][scale=1.0]

==== ドメインの取得
今回ドメインの取得をすることは保留としました。理由としては我々のアプリケーションはフロントエンドとバックエンドは別々に作成しており、一般の利用者から直接見えるのはフロントエンドのドメインになるためです。利用者がソース閲覧した時に見えるドメインまでこだわるのであれば取得するようにしてください。AWS では月額料金を払うことでドメインを取得してA レコードを登録するまでが簡単にできるようになっていますので、そちらは先人の方々のブログ記事を参考にしていただけると幸いです。

==== 固定グローバルIP の取得
AWS EC2 ではインスタンス起動時に自動でグローバルIP が割り振られるようになっていますが、デフォルトで割り振られるグローバルIP は仮のものでインスタンスを停止した時にそのグローバルIP は解放され、もう一度起動した時に異なるものが割り振られるようになっています。ここでは固定グローバルIP を取得することでインスタンスが停止しても常に同じグローバルIP アドレス(もしくはAWS 側で自動生成されるFQDN を)使ってアクセスできるようにするため、Elastic IP で固定グローバルIP を取得してインスタンスに関連付けしておきます。固定グローバルIP はEC2 ダッシュボードの左メニューにあるElastic IP から取得します。Elastic IP の画面で"新しいアドレスの割当" をクリックしてアドレスを割り当てます。今回取得できたアドレスはx.x.x.x とします。
...
==== アプリの構築
grouscope バックエンドアプリはDocker で起動するようにできているのでDocker を使う形で実現していきたいと思います。まずは先程作成したインスタンスに以下のようなコマンドでSSH で接続します。your_aws_private_key はEC2 インスタンス作成時にダウンロードした秘密鍵です。
...
再起動したらもう一度ログインし、Docker をインストールします。以下の手順はubuntu の公式リポジトリからインストールする方法ですが、最新のDocker をインストールしたい場合は公式ページ@<fn>{fn_create_aws_ec2_0001}等を参考にしてください。
...
その対策案のひとつとして、アプリケーションに同じ人からの連続リクエストを禁止するためにユーザ認証機能を加え、それに伴いサーバをHTTPS 化しておくと良いでしょう。プライベートのプロジェクトでお金やユーザ本人の個人情報等の重要なもののやり取り等が発生しないのであれば企業者組織の存在証明までは不要だと思うので無料のSSL/TLS 証明書で充分と考えます。

次はリソース監視に関する設定です。一案としてはAWS のCloudWatch を使用してCPU の使用率が異常に高騰していないか、ディスク使用領域を食い尽くしていないか等の監視をしてサービス停止の原因となる要因を事前に発見する仕組みも導入しておいたほうが良いでしょう。しばらくサービスが安定稼働していたのにいきなりエラーが出始め、調べてみたらディスク領域を使い尽くしていたというのは、私個人よく経験しました。

あとはバックアップに関する設定です。AWS EC2 自体が高可用性を実現しているのでハードウェア故障などによりデータが損失したといった事象はめったに発生しないでしょうがヒューマンエラーやアプリケーションの誤動作により大切なデータが消されてしまう可能性も考えなければなりません。バックアップ方法についてはいくつか手法はありますがS3 を使う方法、スナップショットを使う方法、他インスタンスを建てて安価で大容量なCold HDD (sc1)を使う方法等々、選択肢は色々あります。

その他にもいくつか設定すべき項目はあるかとは思いますが、あとはセキュリティ面に注意しながら実際にまずはサービスをリリースしてみて都度気付いた点を改善していくというやり方でも良いかも知れません。


=== Heroku を使ってみた
次はHeroku を触ってみました。Heroku はPaaS なコンテナシステムをベースとしたアプリケーションのデプロイ及び実行環境です。コンテナということはDocker の時に触れたように永続的にデータが保持されていることが保証されていない環境へのデプロイと同様で、私を含め今までサーバサイドでLinux で環境を構築してアプリケーションをデプロイしてきた人間にとっては少し慣れない環境へのデプロイとなるかもしれません。しかしこの性質がGit のようなバージョン管理システムで管理されているアプリケーションと相性が良く、サーバサイドやインフラ周りをさほど気にせずアプリケーション開発に集中すことができるという洗練されたものとなっているのです。

==== デプロイの構成
Heroku ではアプリケーション本体が配備されるephemeral なコンテナ環境のことをDyno と呼びます。Heroku ではHeroku のGit リポジトリに対してリソースをpush することでSlug コンパイラでアプリケーションがビルドされて圧縮、パッケージ化されてSlug へ送られます。そしてパッケージ化されたアプリケーションはDyno にデプロイされ起動します。ここでなぜHeroku のGit リポジトリにpush した資材が直接Dyno にデプロイされないかというと、Slug コンパイラで事前にアプリケーションをパッケージ化することでアプリケーション(Dyno)のスケールアップ要求が来た時にDyno を増設してSlug にあるパッケージからデプロイするようにすることで、キャッシュとして素早く利用できるようにするためです。このようにHeroku では柔軟なスケールアップ要求にも対応できるようにデプロイプロセスが設計されています。
...
//}
完成です。"heroku create" コマンド実行時に出てきたURL "https://agile-earth-xxxxx.herokuapp.com/" にアクセスするとLaravel のWelcome 画面が確認できます。次にmysql コマンドを使ってMySQL へ接続してLaravel アプリケーションからマイグレーションができているかを確認してみましょう。
...
1 つめのメモリ不足についてはHeroku の仕様として無料プランでは512MB のメモリが定められており、その2倍の1GB までメモリ使用量が到達するとDyno マネージャは問答無用でDyno を停止し、再起動してエラーを返すようになっています。メモリを増やすには料金プランを変更することで対応できます。Heroku の料金プラン@<fn>{heroku_dyno_pricing}を見てみるとstandard-2x まで上げればメモリ1GB が定められ、その2倍の2GB までメモリ使用量が到達しなければdyno が再起動されるということは無さそうです。もしくは負荷の高い解析処理と画像生成処理に関しては外部にサーバを建てて処理を委譲するのが良いかも知れません。
...
3 つめの問題については現状アプリケーションが解析結果画像をアプリケーションサーバの所定ディレクトリに保存するようになっていて、あとで参照できるようにファイル名のパスをDB に保存する方式で組まれています。対策としてまず思い浮かぶのが画像ファイルをblob 型としてDB に保管するように設計を変えることです。もう一つはLaravel の機能を使用して画像をAWS S3 のような外部ストレージサービスに保管する方法です。AWS S3 であれば高可用性で容量制限も気にする必要が無いのでストレージのメンテナンス作業にとらわれることもありません。

ここまでHeroku を触ってきてPaaS は一般的なLinux サーバ及びネットワーク環境を提供してくれるIaaS と異なり、サーバ側はインフラ側を特に気にすること無くアプリケーションの方に意識を集中できるという大きな利点があります。もし我々でLinux サーバを建ててそこにアプリケーションを公開することになったら、まずLinux ディストリビューションは何にするかや定期的にOS のセキュリティパッチは当てられているか、新しいOS がどんどんリリースされて今使っているのがサポート終了になっていないかということを気にする必要が出てきます。が、それらについては完全にHeroku におまかせしておけばよいのです。こういった特徴を考慮してHeroku を使うというのは今のクラウドが主流になってきている時代の中で、とてもアリな選択ではないでしょうか。
...
=== 自宅サーバを使ってみた
デプロイ環境として次は自宅サーバを使う方法について見ていきたいと思います。自宅サーバとなるとクラウドとは異なりハードウェアや設備も自分で用意する必要があります。今回自宅でサーバを構築するために1 台何十万のPC、ネットワーク機器や専用のサーバールームも用意して何千万...とはならないのでご安心ください。
今の所grouscope の開発は本番運用はしていませんし、高いSLA を確保するといったことは考えません。ネットワーク帯域も一般家庭レベルで充分です。PC はお古になって使わなくなったものでも良いので手配し、そこにLinux をインストールして必要なパッケージ類を入れてアプリケーションを起動して公開するというものです。あとはもしあなたが自宅でインターネットを閲覧できる環境にあるというのなら、もう自宅サーバを構築する設備は整っているのです。
...
今回はプログラミング等の作業時にメインで使うPC と違うPC をサーバとして使いますが、あなたがメインで使っているPC をサーバにすることもできます。既にWindows やMac が入っていて、それをアンインストールしたくない場合はVMWare やVirtual Box を使ってそこにLinux を入れてもいいですし、そうするまでもなくDocker でサーバを建ててしまっても良いです。また電気代を節約したい場合はRaspberry Pi のような少電力なボードを使うこともできます。

PC の準備できたらUbuntu 18.04 LTS ですが、USB メモリ等を使ってインストールし、固定IP アドレスの設定を済ませてインターネット接続ができることを確認してください。あとはAWS EC2 の時に実施した手順と同様にDocker とアプリケーションの環境を整えてください。
...
しかしそれではあなたのPC のWeb サーバのコンテンツは表示されないでしょう。それは一般家庭の内側のネットワーク(LAN)と外側のネットワーク(WAN)の内部へアクセスされないようにブロックしているからです。また、ルータにパケットが来た所で、どこにそのリクエストを転送したらよいかということをルータは知りません。ということで「どこにそのリクエストを転送したらよいか」ということを知るために、まずはサーバのプライベートアドレス(LAN のIP アドレス)を調べてみましょう。
...
2: enp0s25: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UP group default qlen 1000
    ...
    ......
//}
すると192.168.1.101 であることがわかります。ルータにポートフォワードの設定をしてこのLinux にパケットが転送されるようにしましょう。ルータの設定方法については各お使いのルータのベンダによって色々異なるので説明は割愛させてもらいます(度々で申し訳ありません<(_ _)>)。お使いのルータの説明書を参考にしていただくか、バッファローやIO データの有名所のルータについては比較的情報も多いのでネットで検索して出てくるかと思います。
...
あなたのお住まいがIP 共有の集合住宅であったりルータの設定を自由に変更できずポートフォワードの設定ができない場合、IaaS なクラウドやVPS 等を使用してそこにVPN サーバを建てて自宅サーバからそのサーバに接続してパケットを転送するようにしたり、自宅サーバからそのサーバにSSH 等で接続してポートフォワードする形でパケットを転送してもらうようにしましょう。そして外部ユーザは外部に作ったIaaS なクラウドやVPS の方にアクセスしてもらうようにします。
このように外部にサーバを構築して、そのサーバと自宅サーバパケットを転送してもらうようにすれば、IP 共有型の集合住宅であったりルータ設定を変更せずとも自宅サーバのコンテンツを外部に公開することができます。もちろんこの方法はルータの設定が自由にできる環境やIP 共有の集合住宅でない場合でも利用可能です。セキュリティ的な観点から外部ユーザに自宅のIP アドレスを知られたくないときなどに利用するのも良いでしょう。
//footnote[chenv_01][2019年8月現在、Docker Hub の無料プランでも1つまでならプライベートリポジトリは作成可能です。有料アカウントであれば更に多くのプライベートリポジトリ作成することもできます。詳細はhttps://hub.docker.com/pricing]
//image[chap05/0063_HomServerWithOuterServer][外部サーバからポートフォワード or VPN で自宅サーバにパケットを転送][scale=0.7]

自分で外部サーバを準備するのが億劫であれば、ポートフォワードを利用して手元のアプリケーションを外部に公開するようなサービスがありますのでそれを利用してみるのも良いかも知れません。
...
==== DDNS の利用
ダイナミックDNS は動的に変わるIP アドレスに対して、DNS レコードのIP アドレスをリアルタイムで更新、管理することにより固定的なホスト名やFQDN を提供するDNS サービスです。有名所のDDNS サービスとしては、例えば以下のようなものがあります。
 * 私的DNS(MyDNS.JP): https://www.mydns.jp/
 * no-ip: https://www.noip.com/
 * Duck DNS: http://duckdns.org/
今回は簡単なWeb 公開くらいの要件なのでDNS のA レコードが引ければ事足りるでしょう。ということで、シンプルにドメインを登録、管理できるDuck DNS を使っていきたいと思います。
...
ドメインの後ろ部分はduckdns.org 固定となり、利用者はその上のホスト名部分を自由に選択できます(ただし既に登録されていないこと)。例えば"http://" の後にa6shogefuga(仮) と入力して"add domain" ボタンを押すとドメインが登録できます。
...
//}
これでDDNS の情報を定期的に更新する設定ができました。これで外部のユーザはいつも同じドメインを指定することであなたの自宅のサーバに接続することができるようになります。
...
$ # "NOTICE: ready to handle connections" メッセージが出るまで待つ
//}
この状態でWeb ブラウザを開きURL http://a6shogefuga/ を指定することで、自宅サーバに接続することができます。

==== 自宅サーバで他にやること
自宅サーバでサービスを公開する場合、重要な情報を使わないサービスだったりSLA を気にする必要のない趣味的なものや検証環境として利用するのが良いでしょう。本番サービスとして重要な個人情報やお金の取引を守るためのセキュリティ、必要な時にしっかりと取引ができるようにSLA の確保などといった責任も伴ってきて自宅内の設備だけで賄うのは厳しい部分が出てくるかも知れません。
具体的な行動としては、例えば定期的にLinux や自宅のルータに脆弱性が無いか情報を受け取るアンテナを高くし続けている必要がアリますし、見つかった場合はアップデートしなければなりません。もちろんアップデートしたことによりライブラリの互換性がなくなり、アプリが動かなくなったりサーバが起動しなくなったりというようなことがあってはいけません。また自宅内のLAN となると自分の作業PC や趣味でやっているゲームのハードをつなげることになるかもしれません。そうするとそれらに対して攻撃されたりや情報を盗み見されないようにするためにDMZ も構築しておいたほうが良いかも知れません。またアプリケーショのバックアップ先やハードウェア故障時もサービスが継続できるように冗長化と予備機の準備、停電時に安全にシャットダウンできるようにUPS の準備や自家発電装置の準備等々考えると大変です。

自宅サーバでサービスを展開する場合は、自分の趣味的なサービスや、失敗しても重大な損害や損失の無いサービスとしておいたほうが賢明です。ただ自宅サーバを構築することでネットワークやLinux のあらゆる知識が増えることになります。勉強するにはとてもよい教材なので、そういった意味で勉強がてら自分の趣味サービスを公開してみるのはありかも知れません。

=== クラウドか自宅サーバか
AWS EC2、Heroku、自宅サーバと試してみて、それぞれの特徴をひとことでまとめると以下の通りです。

 * AWS EC2: Linux が扱えて高カスタマイズ性と高SLA を実現
 * Heroku: アプリケーション開発に集中して瞬時なスケールアウトも視野に
 * 自宅サーバ: 純粋にサーバ構築を楽しみストレージやメモリ、CPU を専有

料金及び料金体系については、AWS EC2 はt2.micro 程度であれば無料枠もあり気軽に利用開始できます。メモリが足りなければ、パフォーマンスは落ちるものの、swap 領域を自分で作成してプランのグレードアップ無しに乗り越えるということも可能です。一方でネットワークが従量課金であったりディスク領域を拡張した時の料金など、見積もりを立てずらい部分もあります。Heroku は今回の我々のようなメモリを多く使う処理を伴うアプリケーションの場合、プランのアップグレードがほぼ必須です。メモリを使いすぎると強制的に再起動がかかってしまうためです。そしてアップグレードしてメモリを増やしたとしてもDyno には永続的なデータは保管できないという特徴があるので、永続的なデータを保管する場合はアドオンやS3 のような外部ストレージの追加で乗り越える必要があります。しかしそれらを追加していくと料金がかさんでいってしまう可能性があるので注意するようにしてください。自宅サーバは趣味や勉強でやる分にはコストが良い一方で本格的な商用環境となると設備を自分で準備する必要があり、その目指すグレード次第では青天井な世界です。数台のPC だけで気軽にやるのであればハードウェアの費用以外は電気代程度で、あなた専用のストレージとメモリ、CPU を自由に使えるようになります。
...
Heroku はLinux に関する知識やOS レベルでのセキュリティ脆弱性については考える必要がありません。アプリケーション本体とそれが使うDB やストレージなどのミドルウェア等に集中できます。しかしサービスを使っているとAWS 上のインフラを使っているように見えるのでAWS で時々発生するメンテナンスによりHeroku も停止する可能性があるかもと個人的に考えています(情報求む)。
自宅サーバについてはネットワーク、OS、アプリケーションからミドルまで自分でやる必要があります。また停電したときもサービス続けようと考えたりしたら自家発電装置を準備したりする必要がありますが、現実的ではありません。またディザスタリカバリのことも考えたりすると、自宅サーバで本格的なサービス運用はハードルが高いかも知れません。

勉強についてはAWS EC2 であればLinux のコマンドやミドルウェアのセットアップ等の知識については勉強することができるでしょう。Heroku は今流行りのコンテナ技術を駆使したサービスで、コンテナの性質について経験が深まるでしょう。Heroku は独特な環境でアプリを動かすので今までLinux でサーバを構築してそこにアプリケーションをデプロイしてきた人にとっては慣れないかも知れませんが、アプリケーション開発の方に集中できる洗練された環境となっています。自宅サーバについてはネットワークからOS の知識、ミドルウェアからアプリの知識まで一通りが経験できます。場合によってはディスク増設などでハードウェアの知識も身につくでしょう。

使うのはクラウドか自宅サーバかということについては、まだスタートしたばかりでアプリケーション開発やサービスに集中したいのであれば、ほぼクラウドで決まりでしょう。今となっては無料で始められるものも多くなっていますし、一定時間内の稼動に無料枠を設けており、お試しの利用であれば料金が発生しないなどのキャンペーンをやっているところもたくさんあります。利用してみて良いなと感じたりサービスが波に乗ってきて本格的に展開していきたいとなった時に料金を払い拡張していく、というスモールスタート戦略を取りやすいのもクラウドならではの魅力です。あとサービス開始の早さが魅力です。クラウドであればハードウェアを準備する手間やOS をインストールする作業も必要ありません。数回のボタンクリックやコマンドの実行ですぐにアプリをデプロイできて動かすことができます。あなたが思いついたアイデアや作品を素早く動くものとしてデプロイし、世界中の人達に公開することができるのです。

ただ、最後にLinux 大好きおじさんからひとこと。趣味には自宅サーバもいいかもよ！
