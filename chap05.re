...
(1)に関しては前項で述べた通りのエラーで、データ永続化のためのマウントディレクトリが他のLaradock を使っているプロジェクトと被たりすると起こりうる問題です。こちらは、やはり前項でも述べたとおりLaradock にも解決方法が用意されていて.env を変更すれば対応できます。残りの2 つは、今回は直面することはありませんでしたが、あらゆる環境設定をbash でやったことにより問題が発生する危険性がありました。(2)については実際にこれが起因でエラーが発生したことはありませんが、エラーが発生しうるリスクはありました。(3)についてはPOSIX なMac とLinux であっても、両者の間にはちょっとした部分で違いがありました。

この項では、これら現状に対する問題点と解決策について経験したことを語って行きたいと思います。
