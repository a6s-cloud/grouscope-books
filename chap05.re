...
==== 自宅サーバで他にやること
自宅サーバでサービスを公開する場合、重要な情報を使わないサービスだったりSLA を気にする必要のない趣味的なものや検証環境として利用するのが良いでしょう。本番サービスとして重要な個人情報やお金の取扱を始めたりすると、それらを守るためのセキュリティ、必要な時にしっかりと取引ができるようにSLA の確保などといった責任も伴ってきて自宅内の設備だけで賄うのは厳しい部分が出てくるかも知れません。
...
自宅サーバでサービスを展開する場合は自分の趣味的なものや、失敗しても重大な損害や損失の無いものにしておいたほうが賢明です。ただ自宅サーバを構築することでネットワークやLinux のあらゆる知識が増えることになります。勉強するにはとてもよい環境なのでそういった目的で勉強がてら自分のサービスを公開してみるのはありかも知れません。
 ...
 * 自宅サーバ: 純粋にサーバ構築を楽しみストレージやメモリ、CPU を専有可

その他、触っていて感じた各サービスの料金及び料金体系、SLA やメンテナンスについて、勉強になるかどうかについての私個人の感想について述べさせていただきます。

まず料金及び料金体系についてはAWS EC2 はt2.micro 程度であれば無料枠もあり気軽に利用を始めることができます。メモリが足りなければパフォーマンスは落ちるものの、swap 領域を自分で作成してプランのグレードアップ無しに乗り越えるということも可能です。一方でネットワークが従量課金であったりディスク領域を拡張した時の料金など、バックアップ目的でスナップショットを取ろうとしたときの料金見積もりを立てずらい部分もあります。Heroku は今回の我々のようなメモリを多く使う処理を伴うアプリケーションの場合、プランのアップグレードがほぼ必須です。swap 領域を作るオプションがないのとメモリを使いすぎると強制的にアプリケーションの停止、再起動がかかってしまうためです。プランをアップグレードしてメモリを増やしたとしてもDyno には永続的なデータは保管できないという特徴があるので、永続的なデータを保管する場合はアドオンやS3 のような外部ストレージの追加で乗り越える必要があります。しかしそれらを追加していくと料金がかさんでいってしまう可能性があるので注意するようにしてください。自宅サーバは趣味や勉強でやる分にはコストが良い一方で本格的な商用環境となると設備を自分で準備する必要があり、その目指すグレード次第では青天井な世界です。数台のPC だけで気軽にやるのであればハードウェアの費用以外は電気代程度で、あなた専用のストレージ領域とメモリ、CPU を使えるようになります。
...
そして結局の所、使うのはクラウドか自宅サーバかということについてはまだスタートしたばかりでアプリケーション開発やサービスに集中したいのであれば、ほぼクラウドで決まりでしょう。今となっては無料で始められるものも多くなっていますし、一定時間内の稼動に無料枠を設けており、お試しの利用であれば料金が発生しないなどのキャンペーンをやっているところもたくさんあります。利用してみて良いなと感じたりサービスが波に乗ってきて本格的に展開していきたいとなった時に料金を払い拡張していく、というスモールスタート戦略を取りやすいのもクラウドならではの魅力です。あとサービス開始の早さが魅力です。クラウドであればハードウェアを準備する手間やOS をインストールする作業も必要ありません。数回のボタンクリックやコマンドの実行ですぐにアプリをデプロイできて動かすことができます。あなたが思いついたアイデアや作品を素早くデプロイし、世界中の人達に公開することができるのです。

ただ最後にLinux 大好きおじさんからひとこと。趣味と勉強には自宅サーバもいいかもよ！
