...
== 環境カスタマイズ
ツイート内容を集計して画像を作成するgrouzcope-batch では内側にwordcloud を使用しておりPyfhon の環境を整える必要がありました。Laradock はもちろんLaravel に特化した環境なのでPython の環境は我々で構築しないといけない。幸いにもLaradock はDebian 系のLinux ディストリビューションを採用<foot>Docker ファイルを追っていくとphp-fpm をベースにしており更に追っていくとDebian を使っている事がわかる<\foot>しているので情報も多く構築は簡単だと感じていた。またMysql 8 を使っていたが、認証方式のデフォルトが変更されていて、Laravel からの認証が通らないなどの事態が発生していた。それらの問題をひとつひとつ解消するために私がとった行動はこれらをbash スクリプトで全て解決しようという試みであった。
[[bash でゲシュタルト崩壊が起きているの図]]

Docker ホスト側からコンテナ側に対してSQL を発行しているが、このようにbash とその標準入力や引数を使う事でコンテナ側のコマンドを呼び出す事ができます<foot>決して図のような書き方を推奨するものではありません</foot>。戦略によっては、一旦ピュアなLaradock のイメージをビルドして、その後に我々で作成したDocker ファイルでそのLaradock のイメージをベースにgrouscope 専用のイメージをビルドする形式も取れましたが、当時はそこまでカスタマイズ規模は大きくならないだろうと思ってbash で小手先な対応をしていました。今となっては黒歴史です。
