 ...
 * build.sh が叩かれる環境のbash バージョンにより動かないことがある
 * その他build 環境が人それぞれ違う(現にMac とLinux 混在環境)ので、それに合わせてbuild.sh 内のコマンドやオプションを変更する必要がある

(1)に関しては前項で述べた通りのエラーで、データ永続化のためのマウントディレクトリが他のLaradock を使っているプロジェクトと被たりすると起こりうる問題です。こちらは、やはり前項でも述べたとおりLaradock にも解決方法が用意されていて.env を変更すれば対応できます。残りの2 つは、今回は直面することはありませんでしたが、あらゆる環境設定をbash でやったことにやり残りの(2)、(3)のような問題が発生する危険性がありました。

他プロジェクト等でlaradock のMySQL 環境が使われていたりするとデータや設定が混在してしまう可能性があります。今まで環境構築をbuild.sh というbash のスクリプトで実行していましたが、これは実行される環境によって動作が異なることがあり現にトキさんの環境では環境構築に失敗する事象が発生していました。私はスクリプトをArch Linux 上で作成していましたが、実行されるのは殆どがMac という状況でした。そういう状況からローカルのbash スクリプトを実行することでメンバの開発環境を構築することに限界を感じていました。
