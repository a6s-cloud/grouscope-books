...
料金及び料金体系については、AWS EC2 はt2.micro 程度であれば無料枠もあり気軽にスモールスタートでサービスを構築できます。メモリが足りなければパフォーマンスは落ちるものの、swap 領域作成などで乗り越えることが可能ですが、ネットワークが従量課金であったりディスク領域を拡張すると料金が上がるなど見積もりを立てずらい部分もあります。Heroku は今回の我々のようなメモリを多く使う処理を伴う場合はプランのアップグレードがほぼ必須です。メモリを使いすぎると強制的に再起動がかかってしまうためです。そして、アップグレードしてメモリを増やしたとしてもDyno には永続的なデータは保管できないという特徴があるので、永続的なデータを保管する場合はアドオンやS3 のような外部ストレージの追加が必要です。しかしそれらを追加していくと更に料金がかかる可能性があります。自宅サーバは趣味や勉強でやる分にはコストが良い一方で本格的な商用環境となると設備を自分で準備する必要があり、そのグレード次第では青天井です。数台のPC だけで気軽にやるのであればPC の費用以外には電気代程度の料金で、充分なストレージ/メモリ/CPU が専有し放題になります。
...
勉強についてはAWS EC2 であればLinux のコマンドやミドルウェアのセットアップ等の知識については勉強することができるでしょう。Heroku は今流行りのコンテナ技術を駆使したサービスで、コンテナの性質についての経験は深まります。Heroku 独特な環境でアプリを動かすので汎用性のある知識はあまり得られないかも知れませんが、アプリケーションの方に集中できる洗練された環境です。自宅サーバについてはネットワークからOS の知識、ミドルウェアからアプリの知識まで一通りが経験できます。場合によってはディスク増設などでハードウェアの知識も身につくでしょう。

結局、使うのはクラウドか自宅サーバかということについては、まだスタートアップしたばかりでアプリケーション開発やサービスに集中したいのであればほぼクラウドで決まりでしょう。今となっては無料で始められるものも多くなっていますし、一定時間内の稼動に無料枠を設けてお試しの利用であれば料金が発生しないなどのキャンペーンをやっているところもたくさんあります。利用してみて良いなと感じたり、サービスが波に乗ってきて本格的に展開していきたいとなった時に料金を払い拡張していく、というスモールスタートな戦略を取りやすいのもクラウドならではの魅力です。あとサービス開始の早さが魅力です。クラウドであればハードウェアを準備する手間やOS をインストールする作業も必要ありません。数回のボタンクリックやコマンドの実行ですぐにアプリをデプロイできて動かすことができます。あなたが思いついたアイデアや作成したアプリケーションを素早く動くものとして形にでき、世の中の人たちに公開することができるのです。
