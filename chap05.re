...
当時デプロイ環境として注目していたのは、IaaS としてAWS のEC2(Elastic Compute Cloud)やGCP のGCE(Compoute Engine)、PaaS として話題になっていたHeroku がありました。一方で古参な人間としてはLinux 環境とネットワーク環境だけ取り揃えられたVPS(Virtual Private Server)で、自分で必要なパッケージをセルフでインストールしてサーバを構築したり、自宅にサーバを建てたりでも良いと考えていました。ここでは、最終的にはまだ決まっていませんが、本番のデプロイ環境について色々試してみた軌跡について語っていきたいと思います。

=== AWS を使ってみた
パブリッククラウドが最近非常に流行っています。Iaas なAWS やAzure、GCP といったプロダクトも複数のカスタマでハードウェアリソースを共有しあい、スモールスタートが可能でコストも削減でき、インスタンス数も短時間で増減できるという柔軟性が企業や個人開発、スタートアップや検証環境等として利用できるサービスを提供しています。その中でもAWS は現在も使っているという組織などが増えてきており、AWS を知らないという技術者はほとんどいなくなっているような状態です。今回はそのAWS を使ってデプロイ環境として試してみました。
