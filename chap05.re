...
== デプロイ
開発環境も決まり、フレームワークも決まり、アプリケーションの開発も進み一通りのアプリケーションの形ができた頃でした。やはりその頃に考えるのはデプロイ環境はどこにするか、といったことが話題に上がってきました。
当時デプロイ環境として注目していたのは、IaaS としてAWS のEC2(Elastic Compute Cloud)やGCP のGCE(Compoute Engine)、PaaS として話題になっていたHeroku がありました。一方で古参な人間としてはLinux 環境とネットワーク環境だけ取り揃えられたVPS(Virtual Private Server)で、自分で必要なパッケージをセルフでインストールしてサーバを構築したり、自宅にサーバを建てたりでも良いと考えていました。ここでは最終的にはまだ決まっていませんが、本番のデプロイ環境について色々試してみた軌跡について概略だけを語っていきたいと思います。

=== AWS を使ってみた
パブリッククラウドが最近非常に流行っています。Iaas なAWS やAzure、GCP といったプロダクトも複数のカスタマでハードウェアリソースを共有しあい、スモールスタートが可能でコストも削減でき、インスタンス数も短時間で増減できるという柔軟性が企業や個人開発、スタートアップや検証環境等として利用できるサービスを提供しています。その中でもAWS は現在も使っているという組織などが増えてきており、AWS を知らないという技術者はほとんどいなくなっているような状態です。今回はそのAWS のIaaS なサービスEC2 を使ってデプロイ環境として仮構築してみました。

==== インスタンスの作成
Amazon Linux、Ubuntu、Red Hat Enterprise Linux、SUSE Linux、Windows Server とEC2 ではLinux やWindows をメインに様々なディストリビューションをインスタンスとして使用できるようになっています。その中でも、前々から世界中で利用されて情報量も多いUbuntu 18.04 を使用することとしました。
//image[chap05/AWSEC2_CreateInstance0000][インスタンスの作成画面(初期)][scale=1.0]

インスタンスの作成画面まできたら、次はインスタンスの作成ボタンを押してインスタンスの作成に進みます。今回は紙面の都合上説明を割愛しますが、Ubuntu 18.04 を選択して後はデフォルト設定のままで作成していきます。インスタンス作成の最後にSSH の秘密鍵をダウンロードするのを忘れないようにしてください。
...
==== ドメインの取得
今回ドメインの取得をすることは保留としました。理由としては、今回フロントエンドとバックエンドのアプリは別々に作成しており、一般の利用者から直接見えるフロントエンドのドメインになるためです。利用者がソース閲覧までした時に見えるドメインまでこだわるのであれば取得するようにしてください。今回は説明を省略しますが、AWS では月額料金を払うことでドメインを取得してA レコードを登録するまでが簡単にできるようになっていますので、そちらは先人の方々のブログ記事を参考にしていただけると幸いです。
...
以上でインスタンスと固定IP の取得は完了しました。次は実際にインスタンスにSSH でつないでgrouscope のバックエンドアプリを構築をしていきます。

1 点補足事項として、今回ドメインの取得をすることは保留としました。理由としては今回フロントエンドとバックエンドのアプリは別々に作成しており、一般の利用者から直接見えるのはフロントエンドのドメイン(FQDN)になるためです。利用者がソース閲覧までした時に見えるバックエンドのドメインまでこだわるのであれば取得するようにしてください。AWS では月額料金を払うことでドメインを取得してレコードを登録するまでが簡単にできるようになっていますので、そちらは先人の方々のブログ記事を参考にしていただけますと幸いです。

==== アプリの構築
Linux 環境とグローバルIP 環境が設定できたので、バックエンドアプリの構築に移ります。バックエンドアプリはDocker で起動するようにできているのでそれを再利用していきたいと思います。まずは先程作成したインスタンスにSSH 接続します。your_aws_private_key はEC2 インスタンス作成時にダウンロードした秘密鍵です。
...
以上で準備は完了です。準備ができたらgrouscope-backend をclone して起動コマンドを叩くだけです。
...
最後にAWS のセキュリティグループ設定でインスタンスに対するHTTP リクエストを許可するようにします。AWS のEC2 のダッシュボードに戻りインスタンスを選択して、セキュリティグループ(今回は"launch-wizard-1")の文字をクリックします。
//image[chap05/AWSEC2_SecurityGroup0001][セキュリティグループ(紙面の都合上表示項目を調整)][scale=1.0]

セキュリティグループを選択した状態で下のインバウンドルールとしてHTTP のルールを追加します。
...
=== 自宅サーバを使ってみた
デプロイ環境の最後として自宅サーバを使う方法です。自宅サーバとなるとハードウェアや設備も自分で用意する必要がアリます。1 台何十万、ネットワークや専用のサーバルームも準備するとなると何千万...とはならないのでご安心ください。
今回はプライベートな環境なので高いSLA を確保するといったことは考えませんし、ネットワーク帯域も一般家庭レベルで実現していきます。ハードウェアは、今あなたが現役で使っているものでもお古になって使わなくなったものでも良いのでPC を用意し、そこにLinux をインストールして必要なパッケージ類を入れてして公開するといったものです。もしあなたが自宅でPC を使っていてインターネットを閲覧できる環境にあるのなら、もう設備は充分に整っているのです。

今回はパッケージのパッケージのインストールやgrouscope のアプリケーション起動に関してはAWS EC2 の時に大方説明したので、ここでは主に自宅のハードウェアやネットワークスイッチと構築したLinux サーバにアクセスできる設定方法について説明していきたいと思います。

==== ハードウェア
まずハードウェアですがPC であれば基本的に何でも良いです。今回私の環境では写真にあるThinkPad を使いました。今メインで使っているThinkPad の1つ前のものになります。
//image[chap05/0061_HomeServer0001][お古になったThinksPad。これをサーバにします][scale=1.0]

今回はこのPC をサーバとして使いますが、既にあなたが使っているコンピュータをサーバにすることもできます。既にWindows やMac が入っていて、それをアンインストールしたくない場合はVMWare やVirtual Box を使ってそこにLinux を入れてもいいですしLinux を入れるまでもなく、そこにDocker でサーバを建ててしまっても良いです。また電気代を節約したい場合はRaspberry Pi を使うこともできます。選択肢はたくさんあるのです。

==== 外からアクセスするためのIP(グローバルIP)
AWS EC2 でインスタンスを作った時、グローバルIP を取得したのを覚えていますでしょうか？このグローバルIP ですが当然みなさんの自宅にも割り当てられています。そのグローバルIP を確認するには以下のコマンドを実行します。
...
IP アドレスが表示されましたでしょうか？それがあなたがの家のグローバルIP アドレスで、そとからあなたの家にアクセスするにはそのIP アドレスを使えばよいのです@<fn>{homeserver_get_first_grobal_ip}。
//footnote[homeserver_get_first_grobal_ip][アパートやマンションのような集合住宅の場合建物1 つで1 つのグローバルIP アドレスである可能性はあります]

==== アクセスする仕組み
そとからアクセスする時のあなたの家のIP アドレスはわかりました。それではあなたの家のPC を起動して、仮にLinux にてDocker でNginx でWeb サーバを立ち上げてください。
//cmd{
$ docker run --rm -p 80:80 -ti nginx
//}

その後にあなたのケータイを開いて4G/LTE 網(自宅のWifi 接続ではない)につないでください。これは家の外にあるPC 端末を想定しています。そして先程出力されたIP アドレスに対してWeb ブラウザからhttp リクエストを飛ばしてみましょう。
//cmd{
http://y.y.y.y
//}

しかしそれでは、あなたのPC のWeb サーバのコンテンツは表示されないでしょう。それは一般家庭の内側のネットワーク(LAN)と外側のネットワーク(WAN)家の内部へアクセスされないようにブロックしているからです。また、ルータにアクセスが来た所でどこにそのリクエストを転送したらよいかということをルータは知りません。ということで「どこにそのリクエストを転送したらよいか」ということを知るために、今回はLinux(Ubuntu 18.04 LTS) であることを想定に、サーバのプライベートアドレスを調べてみましょう。
//cmd{
$ ip a
...
2: enp0s25: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UP group default qlen 1000
    ...
    ......
//}
すると192.168.1.101 であることがわかります。ルータにポートフォワードの設定をしてこのLinux にパケットが転送されるように設定しましょう。ルータの設定方法については各お使いのルータのベンダによって色々異なるので説明は割愛させてもらいます<(_ _)>。お使いのルータの説明書を参考にしていただくか、バッファローやIO データの有名所のルータについては比較的情報も多いのでネットで検索して出てくるかと思います。

ポートフォワード設定のイメージとしては、例えばHTTP(80番ポート) の場合は、ルータのWAN 側のグローバルIP の80 番ポートにパケットが到達したらLAN 側のLinux IP(192.168.1.101) の80 番ポートへパケットを転送するというイメージで設定します。

//image[chap05/0062_HomServerPortForward][ルータのポートフォワードイメージ][scale=0.7]

==== アクセスする仕組み(IP 共有の集合住宅 or ルータ設定を変更できない場合)
あなたのお住まいがIP 共有の集合住宅であったりルータの設定を自由に変更できない場合、IaaS なクラウドやVPS 等を使用してそこにVPN 接続したりポートフォワードする形で自宅内のサーバにパケットを転送してもらうようにしましょう。ルータの設定が自由に変更できないと、自宅内にLinux サーバを構築してもそのサーバにポートフォワードするようにルータに指定することができません。しかし外部にサーバを構築して、そのサーバと自宅内のサーバでポートフォワード(or VPN トンネル)を構築して外部サーバに到達したパケットを自宅内のサーバに転送してもらうようにすれば、自宅のルータ設定を変更せずとも自宅のサーバにパケットを転送することができます。もちろんこの方法はルータの設定が自由にできる環境や、IP 共有の集合住宅でない場合でも利用可能です。セキュリティ的な観点から外部ユーザに自宅のIP アドレスを知られたくないときなどに利用するのも良いでしょう。
//image[chap05/0063_HomServerWithOuterServer][外部サーバからポートフォワードorVPN で自宅サーバにパケットを転送][scale=0.7]

免責事項として、これ以降の説明はIP 共有の集合住宅ではない環境で、比較的自由にルータの設定を変更できる環境での説明になりますのでそのてんはご了承ください。
